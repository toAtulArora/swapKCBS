...
In this Letter, we try to address the limitations of refrence {[}ADD{]}
by providing the robustness curve. We use semidefinite programming
with moment matrices to obtain the aforementioned curve. This is the
first ever robustness curve for a non-contextuality inequality for
single quantum system. Furthermore, using single trapped $^{40}{\rm Ca}^{+}$
ion, we provide an experimental demonstration of the local self-testing
scheme.
